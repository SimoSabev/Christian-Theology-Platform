...
import { ArrowLeft, TreePine, Swords, ChevronDown } from 'lucide-react';
...
function ArgumentTreeInner() {
  ...
  const pickerCategories = useMemo(() => buildPickerCategories(treeKeys), [treeKeys]);
  ...
  const activeArgument = useMemo(() => allArguments.find((a) => a.slug === activeKey), [activeKey]);
  ...
  // Keep the category selector in sync when the active tree changes (e.g. via deep link or cross-link).
  useEffect(() => {
  ...
  return (
    <div className="h-[calc(100vh-4rem)] flex flex-col">
      ...
      <div className="px-4 sm:px-6 lg:px-8 py-4 border-b border-border bg-bg-secondary/50 flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <h1 className="t-h1" style={{ fontSize: 'clamp(1.5rem, 3vw, 2rem)' }}>
              {activeArgument?.name ?? 'Argument Structure'}
            </h1>
          </div>
        </div>
        {/* Legend */}
      ...
      {/* Category + argument selector + cross-link */}
      <div className="px-4 sm:px-6 lg:px-8 py-3 border-b border-border bg-bg-primary flex items-center justify-between gap-3 flex-wrap">
        <div className="flex items-center gap-2 flex-wrap flex-1 min-w-0">
          ...
          {/* Argument dropdown, scoped to the selected category */}
          <div className="relative">
            <select
              value={activeKey}
